...
- [x] Added Telegram login account switching: API exposes public bot id, `/login` clears the current LeadVirt session, redirects the current tab through Telegram OAuth logout for first-party cookie handling, and regular Telegram callbacks remain accepted.
