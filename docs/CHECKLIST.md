...
- [ ] Store the Beget `noreply@leadvirt.com` mailbox password in `/opt/leadvirt/secrets/.env`, verify SMTP authentication, and enable production email OTP.
- [ ] Run a real Beget SMTP inbox delivery smoke, then confirm one-time consumption and `authMode=email` on `leadvirt.com`.
