...
- [x] Added Telegram login account switching: API exposes public bot id, `/login` clears the current LeadVirt session, opens Telegram OAuth logout in a top-level popup and keeps it open for first-party Telegram logout, and regular Telegram callbacks remain accepted.
