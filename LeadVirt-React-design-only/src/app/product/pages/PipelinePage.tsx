...
import { leads as initialLeads, type Lead } from "../data";
...
const LeadCard = React.forwardRef<HTMLDivElement, {
  ...
  isLast: boolean;
}>(function LeadCard({ lead, onAdvance, isLast }, ref) {
  const { go } = useNav();
      ...
      <div className="rounded-2xl bg-zinc-900/60 border border-white/[0.06] backdrop-blur-sm p-4 hover:border-white/[0.12] hover:bg-zinc-900/80 transition-all duration-200">
        ...
        <div className="flex items-start justify-between gap-2 mb-3">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            >
              <DropdownItem onClick={() => go("conversation", { id: lead.id })}>
                Открыть диалог
              </DropdownItem>
              <DropdownItem onClick={() => toast.success("Лид квалифицирован")}>
                Отметить квалифицированным
              </DropdownItem>
              <DropdownSeparator />
              <DropdownItem danger onClick={() => toast("Лид закрыт")}>
                Закрыть лид
              </DropdownItem>
            </Dropdown>
          </div>
...
function KanbanColumn({
  ...
  return (
    <div className="flex flex-col min-w-[292px] max-w-[292px]">
      {/* Column header */}
      <div
        className={cn(
...
export function PipelinePage() {
  ...
  const advanceStage = (id: string) => {
    setLeads((prev) =>
      prev.map((l) => {
        ...
        if (idx === stageOrder.length - 1) return l;
  ...
  const convRate = Math.round((bookedLeads / totalLeads) * 100);
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              ...
              <p className="text-sm text-zinc-500 mt-0.5">
                {totalLeads} лидов · обновлено только что
              </p>
            </div>
