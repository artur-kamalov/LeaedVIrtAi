...
async function main() {
  ...
  try {
    ...
    assert(initial.tenant.name === "Current database name", "Account returned stale request context.");
    assert(initial.description === "Original description", "Account omitted the stored description.");
    ...
    const updated = await service.updateAccount(context, {
    ...
    assert(updated.description === "Updated private profile text", "Description was not normalized.");
    ...
    assert(profile.website === "https://current.example.test/profile", "Concurrent save lost website.");
    assert(notifications.daily === true, "Concurrent save lost notification settings.");
    assert(record(settings.unrelated).retained === true, "Concurrent save erased unrelated settings.");
    assert(profile.retainedProfileField === "keep", "Profile update erased an unknown profile field.");

    const cleared = await service.updateAccount(context, {
      description: null,
      phone: null,
      website: null,
    });
    ...
    for (const privateValue of [
      ...
      assert(!auditJson.includes(privateValue), `Audit payload leaked profile value: ${privateValue}`);
    }
    assert(auditJson.includes("profileFields"), "Audit payload omitted changed profile field names.");
